...
To try an unreleased build, point your MCP client at `build/index.js` instead of `npx`:
...
- `build/index.js` is generated by `npm run build`; rebuild and restart the MCP client after changing the source.
- GUI applications such as Claude Desktop may not inherit your shell `PATH`. If the client reports `spawn node ENOENT`, replace `"node"` with the absolute path printed by `which node`.
- With Claude Code, the same can be registered in one command: `claude mcp add epsg-dev -e EPSG_PACKS=jp,us,uk -- node /absolute/path/to/epsg-mcp/build/index.js`
- `npx @modelcontextprotocol/inspector node build/index.js` opens the local build in MCP Inspector.
